...
import { generateDriverTpContent } from './driver-tp.generator';

export function generateDriverTpLtaContent(submission: SubmissionData): Content[] {
  const content: Content[] = [];
  const formData = submission.formData;
  const assessment = formData.assessment || {};
  
  // Include TP sections (Medical Declaration, History, General Exam, AMT)
  content.push(...generateDriverTpContent(submission));
  
  // Add LTA-specific Vocational Licence section
  content.push({
  ...
  // Overall Result section is handled differently based on purposeOfExam
  content.push({
  ...
  // Check purpose of exam to show appropriate fitness assessment
  if (submission.purposeOfExam === 'AGE_65_ABOVE_TP_ONLY' || submission.purposeOfExam === 'AGE_65_ABOVE_DRIVING_ONLY') {
    if (assessment.fitToDrive !== undefined) {
      ...
      content.push({
        text: assessment.fitToDrive 
          ? 'YES - Patient is fit to drive' 
          : 'NO - Patient is not fit to drive',
        fontSize: 12,
        bold: true,
  ...
  } else if (submission.purposeOfExam === 'AGE_64_BELOW_LTA_ONLY') {
  ...
  } else if (submission.purposeOfExam === 'BAVL_ANY_AGE' && assessment.fitForBusAttendant !== undefined) {
    content.push({
  ...
  } else if (submission.purposeOfExam === 'AGE_65_ABOVE_TP_LTA') {
    // Show both TP and LTA assessments
    if (assessment.fitToDrive !== undefined) {
      content.push({
        text: 'Traffic Police Assessment',
        fontSize: 10,
        bold: true,
        margin: [0, 5, 0, 3] as [number, number, number, number],
      });
    ...
    if (assessment.fitToDrivePublicService !== undefined) {
      content.push({
        text: 'LTA Vocational Assessment',
        fontSize: 10,
        bold: true,
        margin: [0, 5, 0, 3] as [number, number, number, number],
      });
